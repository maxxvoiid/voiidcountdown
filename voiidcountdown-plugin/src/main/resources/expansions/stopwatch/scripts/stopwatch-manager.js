(function () {
    ...
    var TimerFormat = Java.type('voiidstudios.vct.configs.model.TimerFormat');
    ...
    function applyFormattedTitle(bossbar, text) {
        ...
        try {
            componentClass = Java.type('net.kyori.adventure.text.Component');
        } catch (ignored) {}

        if (componentClass && componentClass.isInstance(formatted)) {
            try {
    ...
    var StopwatchManager = {
        ...
        startFromEvent: function (event) {
            ...
            if (!cfg || cfg.getFormat() !== TimerFormat.STOPWATCH) {
